...
const state={selected:null,hidden:new Set(),isolate:false,hardware:false,separation:0,peeling:false,peelStage:8,autoRotate:false};
...
function selectLayer(i){stopPeel();state.selected=i;state.isolate=false;if(i!==null)state.hidden.delete(i);if(focusLayer){focusLayer=false;setCamera('full');}showDetail(i);updateUI();}
function stopPeel(){state.peeling=false;state.peelStage=8;clearInterval(peelTimer);peelTimer=null;}
function setSeparation(v){stopPeel();state.separation=v;focusLayer=false;setCamera('full');updateUI();}
...
function reset(){stopPeel();state.selected=null;state.hidden.clear();state.isolate=false;state.hardware=false;state.separation=0;state.autoRotate=false;focusLayer=false;showDetail(null);setCamera('full');updateUI();}
$('layers').addEventListener('click',e=>{const select=e.target.closest('[data-select]'),vis=e.target.closest('[data-eye]');if(select)selectLayer(Number(select.dataset.select));if(vis){stopPeel();const i=Number(vis.dataset.eye);state.hidden.has(i)?state.hidden.delete(i):state.hidden.add(i);if(state.hidden.has(state.selected)){state.selected=null;state.isolate=false;showDetail(null);}updateUI();}});
$('show-all').addEventListener('click',()=>{stopPeel();state.selected=null;state.hidden.clear();state.isolate=false;showDetail(null);updateUI();});
$('close-detail').addEventListener('click',()=>selectLayer(null));
...
$('hardware').addEventListener('click',()=>{stopPeel();state.hardware=!state.hardware;state.isolate=false;state.selected=state.hardware?5:null;state.hidden.clear();showDetail(state.selected);updateUI();});
$('rotate').addEventListener('click',()=>{state.autoRotate=!state.autoRotate;cameraTween=null;updateUI();});
$('front').addEventListener('click',()=>{state.autoRotate=false;setCamera('front');updateUI();});$('back').addEventListener('click',()=>{state.autoRotate=false;setCamera('back');updateUI();});$('reset').addEventListener('click',reset);
...
async function init3D(){
  ...
  controls=new OrbitControls(camera,renderer.domElement);controls.target.set(0,1.0,0);controls.enableDamping=true;controls.dampingFactor=.07;controls.enablePan=false;controls.minDistance=.48;controls.maxDistance=5.5;controls.minPolarAngle=.36;controls.maxPolarAngle=Math.PI*.73;controls.autoRotateSpeed=.65;
  ...
  suit=buildSuit();
  suit.root.traverse(m=>{if(!m.isMesh)return;m.material=m.material.clone();m.userData.baseColor=m.material.color.clone();m.material.envMapIntensity=m.userData.layer===6?1.15:.48;m.userData.opacity=1;});
  suit.collarKnits.forEach(p=>p.userData.isCollar=true);scene.add(suit.root);
  const floor=new THREE.Mesh(new THREE.CircleGeometry(3,96),new THREE.MeshStandardMaterial({color:0x07080a,roughness:.72,metalness:.12,transparent:true,opacity:.92}));floor.rotation.x=-Math.PI/2;floor.position.y=.008;floor.receiveShadow=true;scene.add(floor);
  ...
  const observer=new ResizeObserver(()=>{const b=$('viewport').getBoundingClientRect();renderer.setSize(b.width,b.height,false);camera.aspect=b.width/b.height;camera.updateProjectionMatrix();composer.setSize(b.width,b.height);});observer.observe($('viewport'));
  ...
  $('scene').addEventListener('pointerup',e=>{
    ...
    const hits=raycaster.intersectObjects(suit.parts).filter(h=>h.object.visible&&h.object.material.opacity>.4);if(hits.length)selectLayer(hits[0].object.userData.layer);
  });
  $('scene').addEventListener('keydown',e=>{
    ...
    if(e.key==='+'||e.key==='=')s.radius*=.90;if(e.key==='-')s.radius*=1.1;s.radius=THREE.MathUtils.clamp(s.radius,.48,5.5);s.phi=THREE.MathUtils.clamp(s.phi,.36,Math.PI*.73);camera.position.copy(controls.target).add(new THREE.Vector3().setFromSpherical(s));
  });
  ...
  function frame(t){
    ...
    for(const m of [...suit.parts,...suit.hardwareParts]){
      const target=desiredOpacity(m);m.userData.opacity+=(target-m.userData.opacity)*ease;m.visible=m.userData.opacity>.012;
      ...
      if(state.hardware&&m.userData.hardware){m.material.color.lerp(new THREE.Color(0xb8cdaa),ease);if(m.material.emissive){m.material.emissive.setHex(0x597747);m.material.emissiveIntensity=.25;}}
      else{m.material.color.lerp(m.userData.baseColor,ease);if(m.material.emissive)m.material.emissiveIntensity=0;}
    }
    ...
    controls.autoRotate=state.autoRotate&&!cameraTween;controls.update(dt);occlusion.enabled=state.selected===null&&!state.hardware&&!state.peeling&&currentSeparation<.005&&state.hidden.size===0;composer.render(dt);frameHandle=requestAnimationFrame(frame);
  }
